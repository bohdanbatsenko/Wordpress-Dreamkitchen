...
if ( ! function_exists( 'redgroupvanna_setup' ) ) :
	...
	function redgroupvanna_setup() {
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'redgroupvanna' ),
			)
		);
...
function redgroupvanna_scripts() {
	...
	//wp_register_script( 'redgroupajax', get_stylesheet_directory_uri() . '/libs/custom.js', false, true );
	//wp_register_script( 'mishaajax', get_stylesheet_directory_uri() . '/libs/mishaloadmore.js', false, true );
	wp_register_script( 'main-js', get_stylesheet_directory_uri() . '/js/app.min.js', false, true );
	
	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
	//wp_enqueue_script('redgroupajax');
	//wp_enqueue_script('mishaajax');
...
function blog_scripts() {
	// Register the script
	wp_register_script( 'red-script', get_template_directory_uri(). '/libs/custom.js', array('main-js'), false, true );
...
function load_posts_by_ajax_callback() {
	...
	if($template === 'very') :
		...
		if ( $blog_posts->have_posts() ) : 
			...
			//print_r($cat_real);	
			//print_r($catID);
			//$cat_real = $cat_real[0]->term_id; 
			//print_r($cat_real[0]->term_id);		

			// i need to combine arrays and loop through that array to get the term id
			// to match cat_ID/
			// cat_real is the issuese

			// нужно соединить массивы и пройтись по каждому, чтобы term_id == cat_ID
			

			foreach ($cat_real as $cat) {
				//$cat_real_id = split(',', $cat->term_id);
			}

			$catmerg = array_merge_recursive($cat_real);
